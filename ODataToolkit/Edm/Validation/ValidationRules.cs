...
using ODataToolkit.Csdl.CsdlSemantics;
        ...
        public static readonly ValidationRule<IEdmVocabularyAnnotation> AnnotationInaccessibleTerm =
            new ValidationRule<IEdmVocabularyAnnotation>(
                (context, annotation) =>
                {
                    ...
                    if (!(term is ODataToolkit.Csdl.CsdlSemantics.IUnresolvedElement) && context.Model.FindTerm(term.FullName()) == null)
                    {
                        context.AddError(
                            annotation.Location(),
                    ...
                    if (appliedFunction != null && !context.IsBad(appliedFunction))
                    {
                        if (appliedFunction.Parameters.Count() != expression.Arguments.Count())
                        {
                            context.AddError(new EdmError(
                                    ...
                                    Strings.EdmModel_Validator_Semantic_IncorrectNumberOfArguments(expression.Arguments.Count(), appliedFunction.FullName(), appliedFunction.Parameters.Count())));
                        }
                    ...
                    foreach (IEdmVocabularyAnnotation annotation in annotatable.VocabularyAnnotations(context.Model))
                    {
                        if (!annotationSet.Add(annotation.Term.FullName() + ":" + annotation.Qualifier))
                        {
                            context.AddError(new EdmError(
                                    ...
                                    Strings.EdmModel_Validator_Semantic_DuplicateAnnotation(EdmUtil.FullyQualifiedName(annotatable), annotation.Term.FullName(), annotation.Qualifier)));
                        }
                    }
                });
