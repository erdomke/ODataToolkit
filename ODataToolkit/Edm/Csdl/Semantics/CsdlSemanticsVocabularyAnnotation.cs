        ...
        private IEdmVocabularyAnnotatable ComputeTarget()
        {
            ...
            else
            {
                ...
                if (targetSegmentsCount == 3)
                {
                    ...
                    UnresolvedOperation unresolvedOperation = new UnresolvedOperation(qualifiedOperationName, Strings.Bad_UnresolvedOperation(qualifiedOperationName), this.Location);
                    return new UnresolvedParameter(unresolvedOperation, parameterName, this.Location);
                }

                return new BadElement(new EdmError[] { new EdmError(this.Location, EdmErrorCode.ImpossibleAnnotationsTarget, Strings.CsdlSemantics_ImpossibleAnnotationsTarget(target)) });
            }
        }
